...
const PAYMENT_METHODS = [
  ...
  { id: 'credit_card', name: 'Credit / Debit Card', icon: '💳', desc: 'Visa, Mastercard, RuPay' },
  { id: 'cod', name: 'Cash on Delivery', icon: '💵', desc: 'Delivery pe payment' },
]
...
export default function CheckoutPage() {
  ...
  const handleOrder = async () => {
    if (!form.name || !form.email || !form.phone || !form.address) return toast.error('Sabhi details bharen')
    ...
    try {
      ...
      const { data } = await createOrder({ customer: form, items, totalAmount: total, paymentMethod })
      clearCart()
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { field: 'name', label: 'Pura Naam *', placeholder: 'Monika Sharma' },
                { field: 'email', label: 'Email *', placeholder: 'email@gmail.com', type: 'email' },
                { field: 'phone', label: 'Phone *', placeholder: '+91 99999 99999' },
                { field: 'city', label: 'Shehar', placeholder: 'Jaipur' },
              ].map(f => (
                <div key={f.field}>
                  <label className="text-sm font-medium text-warm-700 mb-1 block">{f.label}</label>
              ...
              <div className="sm:col-span-2">
                <label className="text-sm font-medium text-warm-700 mb-1 block">Pura Address *</label>
                <textarea rows={3} className="input-field resize-none" placeholder="Ghar no., gali, mohalla, shehar, pin code..."
          ...
          <div className="card p-6">
            ...
            <div className="space-y-3">
              {PAYMENT_METHODS.map(m => (
                <motion.label key={m.id} whileTap={{ scale: 0.99 }}
                  ...
                  <input type="radio" name="payment" value={m.id} checked={paymentMethod === m.id} onChange={() => setPaymentMethod(m.id)} className="hidden" />
                  <span className="text-2xl">{m.icon}</span>
                  <div className="flex-1">
                    <div className="font-semibold text-warm-900 text-sm">{m.name}</div>
                    <div className="text-xs text-warm-400">{m.desc}</div>
                  </div>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === m.id ? 'border-primary-500' : 'border-warm-300'}`}>
        ...
        <div className="card p-6 h-fit sticky top-24">
          ...
          {cart.map(item => (
            <div key={item._id} className="flex gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs text-warm-700 line-clamp-2">{item.name}</p>
          ...
          <div className="border-t border-warm-100 pt-4 mb-4">
            ...
            <div className="flex justify-between font-bold text-lg">
              <span>Total</span>
              <span className="text-primary-600">₹{total >= 500 ? total : total + 50}</span>
            </div>
          </div>
          ...
          <p className="text-xs text-warm-300 text-center mt-3">Aapka order confirm hone par email aayega</p>
        </div>
      </div>
    </div>
  )
